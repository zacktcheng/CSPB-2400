  ...
  if ( error ) {
    ...
    for (int row = 0; row < height; row ++ ) {
      for (unsigned int col = 0; col < width; col ++ ) {
        int i = widthAccu + col;
        unsigned char * imageColors = image -> color[row][col];
        // Re-organize color array into row-col-color order to improve the CPE
        imageColors[COLOR_RED  ] = rarray[i];
        imageColors[COLOR_GREEN] = garray[i];
        imageColors[COLOR_BLUE ] = barray[i];
      }
      widthAccu += width;
    }
    //
...
cs1300bmp_writefile(char *filename, struct cs1300bmp *image)
{
  ...
  for (int row = 0; row < height; row ++ ) {
    for (int col = 0; col < width; col ++ ) {
      ...
      unsigned char * imageColors = image -> color[row][col];
      // Re-organize color array into row-col-color order to improve the CPE
      rarray[i] = imageColors[COLOR_RED  ];
      garray[i] = imageColors[COLOR_GREEN];
      barray[i] = imageColors[COLOR_BLUE ];
    }
    widthAccu += width;
  }
